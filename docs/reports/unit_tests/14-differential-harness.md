...
The complete, unedited output of the whole suite (this unit's own tests are not yet broken
out into their own evidence — see `scripts/test-evidence.sh`) is checked in as
[`evidence_all.md`](evidence_all.md), and the toolchain and lint gates it ran with are in
[`evidence_environment.md`](00-environment/evidence_environment.md). Both are regenerated
