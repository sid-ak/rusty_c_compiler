...
- [02 — The Syntax Tree](02-ast/02-ast.md): the node types the parser builds and the later passes read,
  and the deterministic dump the parser's own tests are written against.
- [03 — Lexer: The Token Model](03-lexer-token-model/03-lexer-token-model.md): the vocabulary — every keyword, operator,
  punctuator, and literal form the language has.
- [04 — Lexer: The Scanner](04-lexer-scanner/04-lexer-scanner.md): turning raw bytes into that vocabulary, including
  every way the bytes can be wrong.
- [05 — Parser: Expressions](05-parser-expressions/05-parser-expressions.md): precedence and associativity, which is the
  part of a parser that is wrong most often and visibly least often.
- [06 — Parser: Statements and Recovery](06-parser-statements-recovery/06-parser-statements-recovery.md): declarations,
  statements, and carrying on sensibly after a syntax error.
- [07 — Semantic Analysis: The Type Model](07-sema-type-model/07-sema-type-model.md): what a value is, how big it is,
  and what it may be used as.
- [08 — Semantic Analysis: The Scope Stack](08-sema-scopes/08-sema-scopes.md): which declaration a name refers to,
  and where each variable will live.
- [09 — Semantic Analysis: The Analyzer](09-sema-analyzer/09-sema-analyzer.md): the thirty-one checks, and everything
  recorded for the code generator to read.
- [10 — Code Generation: The Emitter](10-codegen-emitter/10-codegen-emitter.md): assembly as a document — sections,
  symbols, labels, and directives.
- [11 — Code Generation: Frames and Lowering](11-codegen-frame-and-lowering/11-codegen-frame-and-lowering.md): where every value
