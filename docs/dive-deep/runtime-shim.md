...
the project builds — by `mycc` and by `clang` alike, so both sides of a comparison are exercising the
same runtime:
...
built by `mycc` and one built by `clang`. The reasoning for fixed-arity functions instead of `printf`
