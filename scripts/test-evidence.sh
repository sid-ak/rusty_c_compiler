...
# Writes docs/reports/unit_tests/evidence_all.md: the complete output of the whole suite in one
# `cargo test` invocation, outside every unit's own folder because it isn't any one unit's
# evidence — it's the cross-check that the per-unit runs above add up to the same suite CI runs.
generate_all() {
...

	{
		...
		printf 'The complete, unedited output of `cargo test` — the whole suite in one run — as a\n'
		printf "cross-check against the per-unit evidence beside each unit's own report, not a\n"
		printf 'replacement for it.\n\n'
		printf '## Test Run\n\n'
		printf '```\n$ cargo test\n'
		printf '%s\n' "====================================================================="
		cargo test 2>&1 || printf '(exited %s)\n' "$?"
		printf '```\n'
	} >"$report"
	echo "==> wrote $report"
}
